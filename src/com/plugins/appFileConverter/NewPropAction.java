 ...
 * @author zxy
 */
public class NewPropAction extends AnAction {
    ...
    public void update(@NotNull AnActionEvent event) {
        ...
        //根据类型动态控制Action的隐藏显示
        //yaml类型
        event.getPresentation().setEnabledAndVisible(CommonUtils.containFileType(event, Constant.YAML) && presentation.isEnabled());
    ...
    public void actionPerformed(@NotNull final AnActionEvent event) {
        ...
        ApplicationManager.getApplication().runWriteAction(() -> {
            try {
                ...
                //YAML文件处理
                //新建文件
                final VirtualFile virtualFile = CommonUtils.createVirtualFile(event, file.getNameWithoutExtension(), Constant.PROPERTIES_SUFFIX);

                String yamlContent = Yaml2Props.fromContent(content).convert();
                virtualFile.setCharset(file.getCharset());
                virtualFile.setBinaryContent(yamlContent.getBytes());
